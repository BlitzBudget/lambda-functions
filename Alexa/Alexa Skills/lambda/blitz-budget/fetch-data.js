        ...
        if (slotValues.wallet.heardAs && slotValues.wallet.heardAs !== '') {
            ...
            if (slotValues.wallet.ERstatus === 'ER_SUCCESS_NO_MATCH') {
                ...
                if(utils.isNotEmpty(wallet)) {
                    slotStatus += constants.GET_WALLET_BALANCE + wallet['wallet_balance'].N + ' ' + walletCurrency;
                    // Send a simple card to Alexa on Success
                    responseBuilder.withSimpleCard(constants.CARD_SECURITY_TITLE, constants.CHANGED_DEFAULT_WALLET_SIMPLE_CARD);   
                } else {
            ...
            // Successfully Changed the default wallet then
            if(utils.isNotEmpty(wallet)) {
                slotStatus += constants.GET_WALLET_BALANCE + wallet['wallet_balance'].N + ' ' + wallet['currency'].S;
                // Send a simple card to Alexa on Success
                responseBuilder.withSimpleCard(constants.CARD_SECURITY_TITLE, constants.CHANGED_DEFAULT_WALLET_SIMPLE_CARD);   
            } else {
    ...
    async handle(handlerInput) {
        ...
        if (slotValues.category.heardAs && slotValues.category.heardAs !== '') {
            ...
            if(utils.isEmpty(category)) {
                slotStatus += constants.THE_MESSAGE + slotValues.category.heardAs + constants.CATEGORY_EMPTY + slotValues.category.heardAs;
                shouldEndSession = false;
            } else {
                slotStatus += constants.YOUR_MESSAGE + slotValues.category.heardAs + constants.CATEGORY_BALANCE_ERROR_TWO + category['category_total'].N + ' ' + wallet.currency.S;   
            }
        } else {
            slotStatus += constants.ERR_MESSAGE;
            shouldEndSession = false;
        ...
        if (slotValues.category.heardAs && slotValues.category.heardAs !== '') {
            ...
            if(utils.isEmpty(category)) {
                ...
                if(utils.isEmpty(budget)) {
                    slotStatus += constants.THE_MESSAGE + slotValues.category.heardAs + constants.BUDGET_EMPTY_ERROR + slotValues.category.heardAs;
                    shouldEndSession = false;
                } else {
                    slotStatus += constants.YOUR_MESSAGE + slotValues.category.heardAs + constants.BUDGET_RETRIEVED + budget['planned'].N + ' ' + wallet.currency.S;
                }
            }
        } else {
            slotStatus += constants.ERR_MESSAGE;
    ...
    async handle(handlerInput) {
        ...
        if (slotValues.category.heardAs && slotValues.category.heardAs !== '') {
            ...
            if(utils.isEmpty(category)) {
                ...
                let budget = await blitzbudgetDB.getBudgetAlexaAmount(wallet.sk.S, category.sk.S, currentDate);
                if(utils.isEmpty(budget)) {
                    slotStatus += constants.THE_MESSAGE + slotValues.category.heardAs + constants.BUDGET_NOT_CREATED + category['category_name'].S + constants.BUDGET_NOT_CREATED_TWO + category['category_name'].S ;
                    shouldEndSession = false;
                } else {
                    let budgetBalance = Number(budget['planned'].N) + Number(category['category_total'].N);
                    slotStatus += constants.YOUR_MESSAGE + slotValues.category.heardAs + constants.BUDGET_BALANCE_ONE + budgetBalance + ' ' + wallet.currency.S + constants.BUDGET_BALANCE_TWO;   
                }
            }
        } else {
            slotStatus += constants.ERR_MESSAGE;
    ...
    async handle(handlerInput) {
        ...
        if (slotValues.tag.heardAs && slotValues.tag.heardAs !== '') {
            ...
            if(utils.isEmpty(tagBalance)) {
                slotStatus += constants.THE_MESSAGE + slotValues.tag.heardAs + constants.EMPTY_TAG;
                shouldEndSession = false;
            } else {
                slotStatus += constants.YOUR_MESSAGE + slotValues.tag.heardAs + constants.TAG_MESSAGE + tagBalance + ' ' + wallet.currency.S;   
            }
    ...
    async handle(handlerInput) {
        ...
        if(utils.isEmpty(wallet)) {
            ...
            if(utils.isEmpty(defaultAccount)) {
                ...
                say = constants.DEFAULT_ACCOUNT_SUCCESS + defaultAccount['bank_account_name'].S;
            }
        }
