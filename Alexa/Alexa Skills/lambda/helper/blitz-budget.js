...
const ERROR_ADDING_CATEGORY =
  'Error adding a new category. Please try again later!';
const ERROR_ADDING_GOAL =
  'There was an error while adding a new goal! Please try again later.';
const ERROR_BUDGET_ADD =
  'There was an error while adding a new budget! Please try again later.';
const DEFAULT_ACCOUNT_NOTPRESENT =
  'You do not have a default account in assigned to your wallet!';
const ERROR_ADDING_WALLET =
  'There was an error while adding a new wallet! Please try again later.';
const ERROR_EARNED =
  "We couldn't calculate your earnings at this moment. Please try again later!";
...
const ERROR_BUDGET_AMOUNT =
  'There was an error while updating the budget amount! Please try again later.';
const ERROR_EXPENDITURE =
  "We couldn't calculate your expenditure at this moment. Please try again later!";
const ERROR_ADDING_TRANSACTION =
  'There was an error while adding a new transaction! Please try again later.';
const CHANGING_DEFAULT_WALLET =
  'Oops! there was an error changing the default wallet. Please try again later!';
const ERROR_CHANGING_ACCOUNT =
  'Oops! there was an error changing the default account. Please try again later!';
const ERROR_TRANSACTION_TOTAL =
  "We couldn't calculate your transaction total at this moment. Please try again later!";
 ...
 * Fetch all category
 */
blitzbudgetDB.prototype.getCategoryAlexa = async function (
  walletId,
  categoryName,
  date
) {
 ...
 * Fetch all budget
 */
blitzbudgetDB.prototype.getBudgetAlexaAmount = async function (
  walletId,
  categoryId,
  selectedDate
) {
...
blitzbudgetDB.prototype.getTagAlexaBalance = async function (
  walletId,
  tagName,
  selectedDate
) {
...
blitzbudgetDB.prototype.changeDefaultWalletAlexa = async function (
  userId,
  data,
  currencyName
) {
  ...
  if (utils.isNotEmpty(data)) {
    ...
    for (let i = 0, len = data.length; i < len; i++) {
      let item = data[i];
      if (
        utils.isEqual(item.currency.S.toUpperCase(), currencyName.toUpperCase())
      ) {
        console.log(
...
blitzbudgetDB.prototype.calculateWalletFromAlexa = function (
  allWallets,
  slotValues
) {
  let data = allWallets,
...
blitzbudgetDB.prototype.changeBudgetAlexaAmount = async function (
  walletId,
  budgetId,
  amount,
  currencyName
...
blitzbudgetDB.prototype.addWalletFromAlexa = async function (userId, currency) {
  ...
  return dbHelper
    ...
    .then((data) => {
      console.log(
        'Successfully added the wallet to the DynamoDB. Item count is ' +
          data.length
      );
...
blitzbudgetDB.prototype.addBudgetAlexaAmount = async function (
  walletId,
  categoryId,
  amount,
  currentDate,
...
blitzbudgetDB.prototype.addNewGoalFromAlexa = async function (
  walletId,
  amount,
  goalType,
  monthlyContribution,
...
blitzbudgetDB.prototype.addTransactionAlexaAmount = async function (
  walletId,
  categoryId,
  amount,
  dateId,
  ...
  let currentDate =
    today.getFullYear() +
    '-' +
    ('0' + (Number(today.getMonth()) + 1)).slice(-2);
  let dateObj = await getDateData(walletId, currentDate);
...
blitzbudgetDB.prototype.getEarningsByDateFromAlexa = async function (
  walletId,
  dateId,
  walletCurrency
) {
...
blitzbudgetDB.prototype.getExpenditureByDateFromAlexa = async function (
  walletId,
  dateId,
  walletCurrency
) {
...
blitzbudgetDB.prototype.getTransactionTotalByDateFromAlexa = async function (
  walletId,
  dateId,
  walletCurrency
) {
...
blitzbudgetDB.prototype.addCategoryByDateFromAlexa = async function (
  walletId,
  currentDate,
  categoryName,
  categoryType
...
blitzbudgetDB.prototype.getMatchingWalletAlexa = function (
  allWallets,
  walletCurrency
) {
  let data = allWallets,
...
blitzbudgetDB.prototype.getRecentTransactions = async function (
  walletId,
  dateId,
  walletCurrency
) {
    ...
    .then((data) => {
      ...
      if (utils.isNotEmpty(data)) {
        ...
        for (let i = 0, len = length; i < len; i++) {
          let item = data[i];
          let transactionAmount = Number(item.amount.N);
          let description = utils.isEmpty(item.description)
            ? ''
            : item.description.S;
...
blitzbudgetDB.prototype.changeDefaultAccountAlexa = async function (
  allAccounts,
  accountName
) {
  let events = [],
