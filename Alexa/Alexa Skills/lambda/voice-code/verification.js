...
const SESSION_NOT_VERIFIED =
  'The voice code provided was wrong! If you had forgotten your voice code, <break time="0.05s"/> you could disable and then reenable your Blitzbudget skill in alexa to set a new voice code.';
const LOST_VOICE_CODE =
  'Sorry to hear that! You could disable the blitzbudget skill in Alexa and then re-enable it again.';
const LAST_VOICE_CODE_TRY =
  '<break time="0.10s"/> This is your last try, <break time="0.10s"/> after which you would be signed out of your account for security purposes!';
const SESSION_VERIFIED =
  '<amazon:emotion name="excited" intensity="low">Great! You session has been successfully verified. </amazon:emotion> How can I help you today?';
const VERIFY_VOICE_CODE =
  '<amazon:emotion name="disappointed" intensity="medium"> You need to verify your voice code. </amazon:emotion> Verify by saying <break time="0.20s"/> "Verify Blitz Budget " <break time="0.05s"/> followed by your four digit voice code';
const MAX_RETRIES_EXCEEDED =
  '<amazon:emotion name="disappointed" intensity="medium"> You have exceeded the maximum number of voice code verification tries.</amazon:emotion>  <break time="0.20s"/> Please disable and then re-enable the blitz budget skill.';
...
voiceCodeVerifier.prototype.getAlexaVoiceCode = async function (userId) {
  ...
  return dbHelper
    ...
    .then((data) => {
      console.log(
        'Successfully retrieved the voice code from the DynamoDB. Item count is ' +
          data.length
      );
      return data;
    })
...
function maximumVoiceCodeRetiresExceeded(handlerInput) {
  ...
  const numberOfTimesVoiceVerificationFailed =
    sessionAttributes.numberOfTimesVoiceVerificationFailed;
  console.log(
