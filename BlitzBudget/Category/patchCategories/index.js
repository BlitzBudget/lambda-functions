...
const parameters = [{
        "prmName": "categoryTotal",
        "prmValue": 'category_total'
}
]

exports.handler = async (event) => {
    console.log("updating Category for ", JSON.stringify(event['body-json']));

    await updatingCategories(event).then(function (result) {
        console.log("successfully patched the Categories");
    }, function (err) {
        throw new Error("Unable to patch the Categories " + err);
    });

    return event;
};

function updatingCategories(event) {
    ...
    console.log(" update expression ", JSON.stringify(updateExp), " expression attribute value ", JSON.stringify(expAttrVal), ' expression Attribute Names ', JSON.stringify(expAttrNames), " for the account id ", event['body-json'].accountId);
