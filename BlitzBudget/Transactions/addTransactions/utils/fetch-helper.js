...
async function calculateAndFetchDate(event, walletId, events, docClient) {
    ...
    if (helper.notIncludesStr(dateMeantFor, 'Date#')) {
         ...
         */
        await fetchDate.getDateData(walletId, today, docClient).then(function (result) {
...
async function calculateAndFetchCategory(event, events, docClient) {
    ...
    if (helper.isNotEmpty(categoryName) && helper.notIncludesStr(categoryName, 'Category#')) {
         ...
         */
        await fetchCategory.getCategoryData(categoryId, event, today, docClient).then(function (result) {
            if (helper.isNotEmpty(result.Category)) {
                ...
                events.push(createCategory.createCategoryItem(event, categoryId, categoryName));
            }
        }, function (err) {
