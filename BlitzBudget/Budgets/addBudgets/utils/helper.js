...
let calculateDateMeantFor = (
  ...
  if (isNotEmpty(startsWithDate) && isNotEmpty(endsWithDate)) {
    ...
    if (isFullMonth(startsWithDate, endsWithDate)) {
      ...
      throw new Error(
        'Unable to add the Budget, As the start date and end date must be the same month and year.'
      );
    } else {
      event['body-json'].dateMeantFor = startsWithDate;
      ...
      console.log(
        'Changing the date meant for to %j',
        event['body-json'].dateMeantFor,
        ' and the planned to ',
        event['body-json'].planned
      );
    }
  }
  return dateMeantFor;
