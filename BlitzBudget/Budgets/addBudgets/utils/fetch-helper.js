    ...
    await fetchCategory.getCategoryData(categoryId, event, today).then(
      function (result) {
        if (helper.isNotEmpty(result.Category)) {
          ...
          checkIfBudgetIsPresent = addHelper.createANewCategoryItem(
            event,
            categoryId,
            categoryName,
            events,
    ...
    await fetchDate.getDateData(walletId, today).then(
      function (result) {
        if (helper.isNotEmpty(result.Date)) {
          ...
          dateMeantFor = addHelper.createANewDate(
            dateMeantFor,
            today,
            events,
            event
...
async function calculateAndFetchCategory(
   ...
   */
  ({categoryName, checkIfBudgetIsPresent} = await fetchCategoryIdIfNotProvided(
    event,
    today,
    events,
    checkIfBudgetIsPresent
...
async function calculateAndFetchDate(
  ...
  dateMeantFor = helper.calculateDateMeantFor(
    startsWithDate,
    endsWithDate,
    event,
    dateMeantFor
