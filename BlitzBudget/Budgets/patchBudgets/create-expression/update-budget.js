...
const addCategory = require('../create-parameter/add-category');
...
module.exports.createExpression = (event) => {
  ...
  if (util.isEmpty(event['body-json'])) {
    throw new Error('Unable to update the budget as request body is empty');
  }
  ...
  if (util.isEmpty(expAttrVal)) {
    throw new Error('Unable to update the budget as the parameters are not updatable');
  }
  ...
  return addCategory.createParameter(event, updateExp, expAttrNames, expAttrVal);
};
