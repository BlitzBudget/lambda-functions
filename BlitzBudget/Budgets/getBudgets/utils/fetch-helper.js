...
async function fetchWalletsIfEmpty(walletId, userId, budgetData, docClient) {
    if (helper.isEmpty(walletId) && helper.isNotEmpty(userId)) {
        await wallet.getWalletsData(userId, budgetData, docClient).then(function (result) {
...
async function fetchAllInformationForBudget(events, walletId, startsWithDate, endsWithDate, fullMonth, budgetData, docClient) {
    events.push(budget.getBudgetsItem(walletId, startsWithDate, endsWithDate, budgetData, docClient));
    events.push(category.getCategoryData(walletId, startsWithDate, endsWithDate, budgetData, docClient));
    events.push(date.getDateData(walletId, startsWithDate.substring(0, 4), budgetData, docClient));
    events.push(bankAccount.getBankAccountData(walletId, budgetData, docClient));
    if (!fullMonth) {
        events.push(transaction.getTransactionsData(walletId, startsWithDate, endsWithDate, budgetData, docClient));
    }
