...
async function fetchAllItems(walletId, startsWithDate, endsWithDate, oneYearAgo, overviewData, docClient) {
    let events = [];
    events.push(category.getCategoryData(walletId, startsWithDate, endsWithDate, overviewData, docClient));
    // Get Transaction Items
    events.push(transaction.getTransactionItems(walletId, startsWithDate, endsWithDate, overviewData, docClient));
    // Get Bank account for preview
    events.push(bankAccount.getBankAccountData(walletId, overviewData, docClient));
    // Get Dates information to calculate the monthly Income / expense per month
    events.push(date.getDateData(walletId, oneYearAgo, endsWithDate, overviewData, docClient));
...
async function fetchAllWallets(walletId, userId, overviewData, docClient) {
    if (helper.isEmpty(walletId) && helper.isNotEmpty(userId)) {
        ...
        events.push(wallet.getWalletData(userId, walletId, overviewData, docClient));
    }
    return walletId;

    async function fetchWalletsFromUser(overviewData, docClient) {
        await wallet.getWalletsData(userId, overviewData, docClient).then(function (result) {
