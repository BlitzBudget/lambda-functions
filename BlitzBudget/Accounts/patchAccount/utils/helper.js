...
async function unselectSelectedBankAccount(event, events) {
    if (isNotEmpty(event['body-json'].selectedAccount)) {
        await fetchBudget.getBankAccountItem(event['body-json'].walletId).then(function (result) {
...
async function handleUpdateBankAccounts(events, event) {
    ...
    var params = buildParameterToUpdate(event, updateExp, expAttrNames, expAttrVal);
