...
function updateAccountBalance(record, docClient) {
...
function updateWalletBalance(record, docClient) {
    let pk = record.dynamodb.Keys.pk.S,
        balance = 0,
    ...
    events.push(updateWallet.updateWalletBalanceItem(primaryWalletId, pk, balance, assetBalance, debtBalance, docClient));
}


function updateCategoryTotal(record, docClient) {
    let pk = record.dynamodb.Keys.pk.S,
        balance = 0,
        category;
    console.log("event is %j", record.eventName);
...
function updateDateTotal(record, docClient) {
    let pk = record.dynamodb.Keys.pk.S,
        balance = 0,
...
function updateRelevantItems(event, docClient) {
    let events = [];
    ...
    for (const record of event.Records) {
        try {
            ...
            if (helper.includesStr(sortKey, 'Transaction#')) {
                ...
                updateCategoryTotal(record, docClient);
                updateAccountBalance(record, docClient);
            } else if (helper.includesStr(sortKey, 'BankAccount#')) {
                console.log("Updating the wallet account balance");
                updateWalletBalance(record, docClient);
            } else if (helper.includesStr(sortKey, 'Wallet#') && isEqual(record.eventName, 'INSERT')) {
                ...
                updateDateTotal(record, docClient);
            }
        } catch (ex) {
